...
describe("er-state-account", () => {
  ...
  it("Update VRF (Outside ER)!", async () => {
    let tx = await program.methods.updateVrf().accountsPartial({
      user: anchor.Wallet.local().publicKey,
      userAccount: userAccount,
    })
    .rpc();

    console.log("\nUser Account VRF Updated (Outside ER): ", tx);
    let info = await program.account.userAccount.fetch(userAccount);
    console.log("Random VRF Data: ", info.data.toString());
  });

  it("Delegate to Ephemeral Rollup (Again)!", async () => {
    let tx = await program.methods.delegate().accountsPartial({
      user: anchor.Wallet.local().publicKey,
      userAccount: userAccount,
      validator: new PublicKey("MAS1Dt9qreoRMQ14YQuhg8UTZMMzDdKhmkZMECCzk57"),
    ...
    console.log("\nUser Account Delegated to ER: ", tx);
  });

  it("Update VRF (Inside ER)!", async () => {
    let tx = await program.methods.updateVrf().accountsPartial({
      user: providerEphemeralRollup.wallet.publicKey,
      userAccount: userAccount,
    })
    ...
    console.log("\nUser Account VRF Updated (Inside ER): ", txHash);
  });

  it("Undelegate from Ephemeral Rollup!", async () => {
    let tx = await program.methods.undelegate().accounts({
      user: providerEphemeralRollup.wallet.publicKey,
    })
    .transaction();
