...
pub struct ScheduleNextCrank<'info> {
    #[account(
        mut,
        seeds = [b"pet"],
        bump = pet.bump,
    ...
    /// CHECK: The TukTuk task account to be created. Derived from seeds [b"task", task_queue.key(), task_id.to_le_bytes()] under the tuktuk program.
    #[account(mut)]
...
pub fn schedule_next_crank(ctx: Context<ScheduleNextCrank>, task_id: u16) -> Result<()> {
    ...
    // We will compile the transaction to run `increment_counter` (our decay tick)
    ...
    // Schedule to trigger 60 seconds in the future
    ...
    let args = QueueTaskArgsV0 {
        ...
        transaction: TransactionSourceV0::CompiledV0(CompiledTransactionV0 {
            ...
            accounts: vec![
                ...
                system_program.key(),
                tuktuk_program.key(),
                crate::id(), // our program id
            ],
            instructions: vec![CompiledInstructionV0 {
                program_id_index: 7, // our program index
