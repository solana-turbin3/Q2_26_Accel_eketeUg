...
use mpl_core::{
    ...
    instructions::{
        UpdatePluginV1CpiBuilder, AddCollectionPluginV1CpiBuilder,
...
pub fn handler(ctx: Context<Unstake>) -> Result<()> {
    ...
    let total_staked_time = current_timestamp.checked_sub(staked_timestamp).ok_or(ErrorCode::InvalidTimestamp)?;
    let total_staked_days = total_staked_time.checked_div(SECONDS_PER_DAY).ok_or(ErrorCode::InvalidTimestamp)?;
    require!(total_staked_days >= ctx.accounts.config.freeze_period as i64, ErrorCode::FreezePeriodNotElapsed);

    // Calculate outstanding rewards time
    let reward_time = current_timestamp.checked_sub(last_claimed_timestamp).ok_or(ErrorCode::InvalidTimestamp)?;
    let reward_days = reward_time.checked_div(SECONDS_PER_DAY).ok_or(ErrorCode::InvalidTimestamp)?;
