...
fn test_full_flow() {
    ...
    // The PDA for the payer (the user we are whitelisting)
    ...
    let ix = Instruction::new_with_bytes(
        ...
        program::accounts::AddToWhitelist {
            admin: payer.pubkey(),
            whitelist_entry: user_whitelist_pda,
    ...
    let build_transfer_ix = |source: &Keypair, mint_kp: &Keypair, src: Pubkey, dst: Pubkey| {
        ...
        // Order: extra_account_meta_list, then TLV-registered extras (whitelist_entry_pda), then hook program ID.
        ix.accounts
            .push(AccountMeta::new_readonly(extra_meta_pda, false));
        ix.accounts
            .push(AccountMeta::new_readonly(user_pda, false));
        ix.accounts
